...
def test_scheduler_enqueues_hochi_discovery_into_default_queue() -> None:
    settings = _create_settings()
    provider = RQQueueProvider(
        settings,
        _create_connection(),
    ...
    assert result.status is SchedulerRunStatus.ENQUEUED
    assert result.hochi_discovery_job_id is not None

    job = provider.get_queue(QueueName.DEFAULT).fetch_job(result.hochi_discovery_job_id)

    assert job is not None
    assert job.func_name == ("project_g.interfaces.workers.jobs.discover_hochi_giants_news")
    assert job.kwargs == {"max_items": 5}


def test_hochi_discovery_uses_five_minute_time_bucket() -> None:
    settings = _create_settings()
    provider = RQQueueProvider(
        settings,
        _create_connection(),
    ...
    assert first.hochi_discovery_job_id is not None
    assert second.hochi_discovery_job_id == first.hochi_discovery_job_id
    assert third.hochi_discovery_job_id != first.hochi_discovery_job_id
    ...
    hochi_jobs = [
        ...
        if job.func_name == ("project_g.interfaces.workers.jobs.discover_hochi_giants_news")
    ]
...
def test_scheduler_enqueues_priority_recovery_into_default_queue() -> None:
    settings = _create_settings()
    provider = RQQueueProvider(
        settings,
        _create_connection(),
    ...
    assert result.status is SchedulerRunStatus.ENQUEUED
    assert result.priority_recovery_job_id is not None

    job = provider.get_queue(QueueName.DEFAULT).fetch_job(result.priority_recovery_job_id)

    assert job is not None
    assert job.func_name == ("project_g.interfaces.workers.jobs.recover_pending_news_priority")
    assert job.kwargs == {"limit": 5}


def test_priority_recovery_uses_five_minute_time_bucket() -> None:
    settings = _create_settings()
    provider = RQQueueProvider(
        settings,
        _create_connection(),
    ...
    assert first.priority_recovery_job_id is not None
    assert second.priority_recovery_job_id == first.priority_recovery_job_id
    assert third.priority_recovery_job_id != first.priority_recovery_job_id
    ...
    recovery_jobs = [
        job
        for job in queue.jobs
        if job.func_name == ("project_g.interfaces.workers.jobs.recover_pending_news_priority")
    ]
