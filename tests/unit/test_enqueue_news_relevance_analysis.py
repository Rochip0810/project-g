...
from project_g.application.news.enqueue_relevance_analysis import (
    NEWS_RELEVANCE_WORKER_FUNCTION,
    EnqueueNewsRelevanceAnalysis,
    NewsRelevanceAnalysisNotPendingError,
)
...
_ANALYSIS_ID = UUID("5edff033-b854-47c0-a3bf-b627688fb89d")
_INTAKE_ID = UUID("3b8eab24-cf9e-40bb-8aac-dc2760a81529")
...
def test_pending_analysis_is_enqueued() -> None:
    ...
    service = EnqueueNewsRelevanceAnalysis(queue_provider=provider)

    snapshot = service.execute(_pending_analysis())

    assert provider.queue_name is QueueName.DEFAULT
    assert provider.function_path == NEWS_RELEVANCE_WORKER_FUNCTION
    assert provider.args == [str(_INTAKE_ID)]
    assert provider.job_id == (f"news-relevance-{_ANALYSIS_ID}")
...
def test_analyzed_analysis_cannot_be_enqueued() -> None:
    analysis = _pending_analysis().record_analyzed(
        ...
        reason="Highly relevant Giants news.",
        updated_at=_CREATED_AT,
    )

    service = EnqueueNewsRelevanceAnalysis(queue_provider=FakeQueueProvider())
